// Se ejecuta de inmediato al cargar el script — si no hay sesión, corta aquí
...
function actualizarResumen() {
    // Se vuelve a leer del localStorage para reflejar el saldo más reciente
...
function navegarA(seccion) {
    ...
    // Datos frescos para que el saldo mostrado en los formularios esté actualizado
    const usuario = obtenerUsuarioActual();
...
function cargarTransacciones() {
    ...
    // slice(-10) toma los últimos 10, reverse() los muestra del más reciente al más antiguo
...
function pagarServicio() {
    // Se usa una bandera para validar todos los campos a la vez y mostrar todos los errores juntos
    let esValido = true;
...
function generarCertificado() {
    ...
    document.getElementById("certTitular").textContent =
        `${usuario.nombres} ${usuario.apellidos}`;
    // Si el tipoId no está en el mapa, muestra el código tal cual (ej: "CE")
...
function manejarImportacion(inputEl) {
    ...
    // importarDatosJSON es asíncrona (retorna una Promesa); .then = éxito, .catch = error
...
// En móvil, tocar el contenido principal cierra el sidebar si estaba abierto
